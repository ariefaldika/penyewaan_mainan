...
if (isset($_POST['login'])) {
    ...
    if (mysqli_num_rows($login) > 0) {
        # code...
        while ($a = mysqli_fetch_array($login)) {
            if ($a['level'] == 1) {
                # code...
                $_SESSION['admin'] = $username;
                echo "<script>window.location='home.php';</script>";
            }else{
                echo "<script>alert('level tidak ditemukan');window.location='login.php'</script>";
            }
        }
    }else{
        echo "<script>alert('user tidak ditemukan');windows.location='login.php'</script>";
    }
}
